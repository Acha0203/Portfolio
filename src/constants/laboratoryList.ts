...
const laboratorySeeds: LaboratorySeed[] = [
  {
    ...
    description: {
      ja: [
        ...
        '5. ダイアログ下部にある「SAVE」をクリックすると、現在の設定と盆栽データ、経過時間をブラウザに保存できます。盆栽の栽培には非常に時間がかかるため、頻繁にデータを保存しておきましょう。',
        '6. ダイアログ下部にある「LOAD」をクリックすると、「SAVE」で保存した状態から盆栽の栽培を再開できます。保存時の設定も一緒に復元されます。',
        '7. ダイアログ下部にある「EXPORT」をクリックすると、ブラウザに保存したデータを JSON および OBJ 形式のファイルとしてダウンロードできます。OBJ ファイルは Blender にインポートすることができます。',
        '8. ダイアログ下部にある「IMPORT」をクリックすると、「EXPORT」でダウンロードした JSON ファイルをブラウザに読み込み、その状態から盆栽の栽培を再開できます。保存時の設定も一緒に復元されます。',
        ...
        '11. 各種設定を変更した後、ダイアログ下部にある「APPLY SETTINGS」をクリックすると、盆栽と経過時間は保持されたまま変更が適用されます。ただし、「Ovule Size」を変更すると固着済みの胚珠の大きさも変わるため、盆栽の見た目が変化します。なお、「LOAD」または「IMPORT」で盆栽データを復元すると、設定も保存時のものに戻ります。',
      ],
      en: [
