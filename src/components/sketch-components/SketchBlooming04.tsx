...
const sketch: Sketch = (p5) => {
  ...
  p5.draw = () => {
    ...
    for (let r = 0; r < p5.TAU; r += p5.PI / 8) {
      const angle = r + (Math.cos(p5.frameCount / 50) / 3) * direction;
      const length = p5.noise(p5.frameCount / 90) * 360;
      const x = Math.tan(Math.cos(angle)) * length + p5.width / 2;
      const y = Math.tan(Math.sin(angle)) * length + p5.height / 2;
